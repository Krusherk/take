...
export function ParticipantEligibilityPanel({ campaignId, request }: { campaignId: string; request: TakeApiClient["request"] }) {
  ...
  return (
    <section className={`participant-eligibility participant-eligibility--${assessment.status.toLowerCase()}`}>
      <header><div><span className="eyebrow">ELIGIBILITY STATUS</span><h2>{assessment.status === "ELIGIBLE" ? "YOU'RE ELIGIBLE." : assessment.status === "NEEDS_REVIEW" ? "WE NEED MORE INFORMATION." : "YOU'RE NOT CURRENTLY ELIGIBLE."}</h2><p>{assessment.status === "ELIGIBLE" ? "You can give one TAKE when nominations are active. Your evidence score does not change its weight." : `TAKE verified ${assessment.totalPoints} of ${data.policy.requiredTotalPoints} required points. Missing evidence can be reviewed before lock.`}</p></div><span className={`status-chip status-chip--${assessment.status.toLowerCase()}`}>{assessment.status.replaceAll("_", " ")}</span></header>
      {assessment.status === "ELIGIBLE" ? <details className="participant-eligibility__evidence"><summary>Why am I eligible? View evidence and integrity checks</summary><EligibilityEvidenceSummary categories={assessment.categories as EligibilityCategoryScore[]} /><IntegritySummary {...assessment.integrity} /></details> : <><EligibilityEvidenceSummary categories={assessment.categories as EligibilityCategoryScore[]} /><IntegritySummary {...assessment.integrity} /></>}
